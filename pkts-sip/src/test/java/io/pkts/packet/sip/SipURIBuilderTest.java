...
public class SipURIBuilderTest {
    ...
    public void testBasicBuild() throws Exception {
        SipURI sipURI = SipURI.with().host(this.host).build();
        ...
        sipURI = SipURI.with().port(5098).user("nisse").host(this.host).build();
        ...
        sipURI = SipURI.with().port(5060).user("nisse").useUDP().host(this.host).build();
    ...
    public void testSips() throws Exception {
        final SipURI sipURI = SipURI.with().secure().host(this.host).build();
        assertThat(sipURI.toBuffer().toString(), is("sips:example.com"));
    }

    @Test
    public void testMessingWithParameters() throws Exception {
        final SipURI sipURI = SipURI.with().host(this.host).parameter("hello", "world").build();
