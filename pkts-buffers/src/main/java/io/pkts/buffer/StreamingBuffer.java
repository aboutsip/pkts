...
public abstract class StreamingBuffer extends BaseBuffer {
  /**
  ...
  protected static final String CANNOT_WRITE_TO_AN_INPUT_STREAM_BUFFER = "Cannot write to an InputStreamBuffer";
  protected static final String NOT_IMPLEMENTED_JUST_YET = "Not implemented just yet";
  ...
  protected StreamingBuffer(int bufferCapacity) {
   ...
   * @throws IOException
   */
  protected int internalReadBytes(final int length) throws IOException {
   ...
   * @return
   */
  protected int getLocalWriterIndex() {
   ...
   * @return
   */
  protected int getLocalReaderIndex() {
  ...
  private int getAvailableLocalWritingSpace() {
    // TODO(epall): Shouldn't this be careful about readerIndex?
   ...
   * @return
   */
  protected int getAvailableLocalReadingSpace() {
   ...
   */
  protected int readFromStream(final long length) throws IOException {
      ...
      while (total < length && actual != -1) {
          ...
          actual = actuallyReadFromStream(localIndex, readAtMost);
  ...
  protected abstract int actuallyReadFromStream(int offset, int length) throws IOException;
  ...
  public abstract byte getByte(final long index) throws IndexOutOfBoundsException, IOException;
...
//        }
  ...
  /**
   * Fetch the next <code>length</code> bytes from the stream as a {@link Buffer}.
   *
   */
  @Override
  public Buffer readBytes(final int length) throws IndexOutOfBoundsException, IOException {
      ...
      while (index < length) {
          ...
          readIntoBuffer(localIndex, buf, index, readAtMost);
  ...
  protected abstract void readIntoBuffer(int sourceOffset, byte[] buf, int offset, int length);
   ...
   * @throws IOException
   */
  protected void checkIndex(final long index) throws IndexOutOfBoundsException {
